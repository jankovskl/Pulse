...
function WeightChart({ series }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
      ...
      {ticks.map((g) => (
        <g key={g}>
          <line x1={PAD_L} y1={y(g)} x2={W - PAD_R} y2={y(g)} stroke="#FFFFFF0D" strokeWidth="1" />
          <text x={PAD_L - 6} y={y(g) + 3} textAnchor="end" fontSize="9" fill="#6E6E7A">
            {Math.round(g * 10) / 10}
          </text>
        </g>
      ))}
      ...
      {values.map((v, i) => (
        <circle
          ...
          fill={i === values.length - 1 ? 'var(--color-accent)' : '#0B0B12'}
    ...
    <Screen activeTab="progress">
      <div className="flex flex-col gap-5">
        ...
        <div className="relative">
          <span className="mb-1.5 block text-[14px] font-medium text-[#C9C9D6]">Exercise</span>
          <button
        ...
        <div className="flex flex-col gap-2.5 rounded-[20px] bg-surface px-4 py-4 outline outline-1 outline-white/10">
          <div className="flex items-center justify-between">
          ...
          {leaderboardFor(current, bestWeight)
            .slice(0, 4)
            .map((r) => (
              <div key={r.name} className="flex items-center gap-3">
                ...
                <Avatar initials={initialsOf(r.name)} color={r.color} size={26} />
        ...
        <div className="flex flex-col gap-2.5 rounded-[20px] bg-surface p-4 outline outline-1 outline-white/10">
          <div className="flex items-center justify-between">
          ...
          {exSessions.length === 0 && (
            <div className="flex flex-col items-center gap-1 py-5 text-center">
              <CalendarCheck size={18} color="#6E6E7A" />
              <span className="text-[12px] text-faint">No sessions yet for {current}</span>
            </div>
          )}
          {exSessions.map((s, i) => (
            <div key={i} className="flex items-center gap-3 border-t border-white/5 pt-2.5 first:border-t-0 first:pt-0">
              <div className="flex flex-1 flex-col leading-tight">
